...
class Expert(BasePolicy):
    ...
    def _predict(self, states, deterministic=False):
        ...
        #print(robot_body.rotation_vector, robot_pos, robot_angle / math.pi)
        #print(diff, diff.angle_degrees, diff.angle / math.pi, robot_angle / math.pi)
        ...
        while i > 0:
            ...
            if d > 0.01:
                break
            cnt_unmoved += (self.history[i]['action'][0] == RA.UP)
            i -= 1

        if target_dist >= 0.55 and cnt_unmoved >= 3:
            act_flags[0] = RA.DOWN

        #print(target_dist, cnt_down, cnt_unmoved, act_flags)
